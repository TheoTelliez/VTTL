...
class Connexion {
    ...
    public function __construct($db) {
        ...
        $this->toutVider = $db->prepare("truncate table connexions"); //le TRUNCATE TABLE `nom_de_la_table` sert à vider une table sans pour autant supprimer sa structure                
        ...
        $this->rechercher = $db->prepare("select idco, dateco, emailco from connexions where dateco like :recherche order by dateco");
    }
    ...
    public function toutVider() {
        $this->toutVider->execute();
        if ($this->toutVider->errorCode() != 0) {
            print_r($this->toutVider->errorInfo());
        }
        return $this->toutVider->fetchAll();
    }
    ...
    public function rechercher($recherche) {
        $listerecherchecommentaire = $this->rechercher->execute(array(':recherche' => '%' . $recherche . '%'));
